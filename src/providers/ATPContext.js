...
function ATP_ContextProvider({ children }) {
  const [state, dispatch] = useReducer(reducer, initialState)

  return (
    <ATP_StateContext.Provider value={state}>
      <ATP_DispatchContext.Provider value={dispatch}>{children}</ATP_DispatchContext.Provider>
    </ATP_StateContext.Provider>
  )
}

export default ATP_ContextProvider
export const useATP_StateContext = () => useContext(ATP_StateContext)
export const useATP_DispatchContext = () => useContext(ATP_DispatchContext)
